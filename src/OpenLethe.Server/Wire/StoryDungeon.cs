...
public sealed class AcquiredEgogifts
{
    ...
    public long un;
    public long ul;
    // Fusion origin id: set only on the "super" combine-fusion gifts (9992/9993), which
    // carry the id of a material they inherited from (e.g. 9993 -> oid 9053). Nullable/
    // omit-when-null so every ordinary gift serializes exactly as before (id/pids/un/ul,
    // no oid key) and only the fused gifts emit `oid` last, matching the capture.
