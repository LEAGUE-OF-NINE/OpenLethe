...
public sealed class Element
{
    ...
    public long id;
    public long num;
    // Nullable/omit-when-null (like RemainRewardEvent.pool_v2/v3, AcquiredEgogifts.oid):
    // the MD exit-reward capture's Element entries (mirrordungeon-07-extreme.json
    // rewardElements, and the Preview/Acquire exit-reward rewardList) never carry a `tags`
    // key at all - always `= new()` would wrongly add one. Set explicitly where a real
    // caller needs it.
