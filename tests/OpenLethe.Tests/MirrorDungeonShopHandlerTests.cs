...
public class MirrorDungeonShopHandlerTests(PostgresFixture db)
{
    ...
    public async Task PurchaseHealMirrorDungeon_Idx1_HealsAllUnits()
    {
        ...
        // Full heal (capture-verified): ch is SET to 10000 (the max scale), not added to.
        Assert.Equal(10000, dul[0].GetProperty("ch").GetInt64());
        Assert.Equal(15, dul[0].GetProperty("cm").GetInt64());
        Assert.Equal(10000, dul[1].GetProperty("ch").GetInt64());
    }
    ...
    public async Task PurchaseEgoGiftMirrorDungeon_BuysFromEgpool()
    {
        ...
        save.currentInfo.shop.slots = new() { new ShopSlot { t = "eg", id = 9001, s = 1 } };
        ...
        // Buying flips the slot's s from 1 to 0 (sold out); the slot stays in place.
        var boughtSlot = result.GetProperty("shopInfo").GetProperty("slots")[0];
        Assert.Equal(9001, boughtSlot.GetProperty("id").GetInt64());
        Assert.Equal(0, boughtSlot.GetProperty("s").GetInt64());
        ...
        Assert.Single(stored.currentInfo.shop.slots);
        Assert.Equal(0, stored.currentInfo.shop.slots[0].s);
    }
    ...
    public async Task SelectFormationMirrorDungeon_BuildsDulAndSetsStartBufPoint()
    {
        ...
        // 80 = MdStartBuffs.BasePoint(60, dungeon-7 static data) + MdEgoData.DetectThemeFloorDefaultPoint(20) -
        // capture-verified (cycle8 start-of-run task), replaces the old hardcoded 120.
        Assert.Equal(80, ci.GetProperty("startBufPoint").GetInt64());
        ...
        Assert.Equal(0, unit.GetProperty("es")[0].GetProperty("idx").GetInt64());

        var stored = AccountFields.Get<MirrorOriginSaveInfo>((await GetAccount(f, name)).MdSaveInfo)!;
        Assert.Equal(80, stored.currentInfo.startBufPoint);
