...
public class MdEventManagerTests
{
    ...
    [Fact]
    public void ApplyResultForm_StartBattleAbnormality_ReturnsStartBattleId()
    {
        var save = new MdEventSave(NewSave());
        var rf = new ResultForm { resultEffect = "StartBattle_Abnormality", startBattleId = 2060008 };

        var nid = MdEventManager.ApplyResultForm(rf, null, save);

        Assert.Equal(2060008, nid);
    }

    [Fact]
    public void ApplyResultForm_StartBattleAbnormality_OldCamelCaseFieldAlone_StaysUnresolved()
    {
        // Guards the additive nature of the fix: the pre-existing nextBattleId (lowercase d)
        // field is deliberately never bound from real JSON (see MdEvent.cs NOTE) and the
        // StartBattle_Abnormality arm must not fall back to it.
        var save = new MdEventSave(NewSave());
        var rf = new ResultForm { resultEffect = "StartBattle_Abnormality", nextBattleId = 555 };
    ...
    [Fact]
    public void ResultForm_JsonBinding_StartBattleIdBindsFromRealNextBattleIDKey()
    {
        // This is what actually broke: nextBattleID (capital ID) is the real static-data key;
        // the old camelCase-attributed nextBattleId field never matched it.
        var options = new System.Text.Json.JsonSerializerOptions { IncludeFields = true };
        var json = "{\"resultEffect\":\"StartBattle_Abnormality\",\"nextBattleID\":2060008}";

        var rf = System.Text.Json.JsonSerializer.Deserialize<ResultForm>(json, options);

        Assert.NotNull(rf);
        Assert.Equal(2060008, rf!.startBattleId);
    }
    ...
    // RecomputeEgmlos (hidden-gift level-cap bumps) coverage moved to MdRulesSharedTests
    // (SharedRules.RecomputeEgmlos on Run) - Task 12 deleted the wire-typed
    // MirrorDungeonMapEndpoints.RecomputeEgmlos these tests used to pin.
}
