...
public class MirrorDungeonEventsHandlerTests(PostgresFixture db)
{
    ...
    public async Task UpdateMapNode_NodeEidPath_ResolvesEventViaDungeonMapNode()
    {
        ...
        using (var doc = JsonDocument.Parse(await resp.Content.ReadAsStringAsync()))
        {
            ...
            // pce started empty, so the appended entry is the only (and first) one.
            var pce = result.GetProperty("prevChoiceEvent").EnumerateArray().ToList();
            Assert.Single(pce);
            Assert.Equal(800202, pce[0].GetProperty("nei").GetInt64());
            Assert.Equal(0, pce[0].GetProperty("cs").GetInt64()); // echoes the request's cs, not a hardcoded sentinel
        ...
        Assert.Equal(0, stored.currentInfo.pce[0].cs);
    }
    ...
    public async Task UpdateMapNode_PceNeiPath_ResolvesEventViaPreviousChoice()
    {
        ...
        var stored = AccountFields.Get<MirrorOriginSaveInfo>((await GetAccount(f, name)).MdSaveInfo)!;
        // Appended at the tail (oldest-first): the pre-existing entry for 805001 stays at
        // pce[0], the newly-processed step lands at pce[1].
        Assert.Equal(2, stored.currentInfo.pce.Count);
        Assert.Equal(805001, stored.currentInfo.pce[0].nei);
        Assert.Equal(805002, stored.currentInfo.pce[1].nei);
    }

    // Real static-data: abnormality-event id 90107402 (personalityEvent, COIN_EVENT),
    // eventResults[0]=SUCCESS grants hidden ego-gift 993003 (+2 mlos bump, see
    // MirrorDungeonMapEndpoints.HiddenGiftLevelBump). Asserts the mlos re-apply the
    // review found missing test coverage for: mlos should become sbmlos + egmlos.
    [SkippableFact]
    public async Task UpdateMapNode_GrantsHiddenGift_RecomputesEgmlosIntoMlos()
    {
        db.RequireDb();
        await using var f = new DbWebAppFactory(db.ConnectionString);
        var (jwt, name) = await NewAccount(f);
        ...
        var save = AccountFields.Get<MirrorOriginSaveInfo>((await GetAccount(f, name)).MdSaveInfo)!;
        save.dungeonMap.ns.Add(new Ns { f = 0, s = 1, nid = 202, e = 1, eid = 90107402 });
        save.currentInfo.cn.nid = 202;
        save.currentInfo.pce.Clear();
        save.currentInfo.efs.sbmlos = 3;
        save.currentInfo.dul.Add(new Dungeonunitlist1 { pid = 7, ch = 10000, cm = 0 });
        await SetSave(f, name, save);

        // cs (coinState) = 0 selects eventResults[0] = SUCCESS -> GetConfirmedEgogift 993003.
        var resp = await client.PostAsJsonAsync("/api/UpdateMirrorDungeonMapNode", Body(jwt, new
        {
            currentnode = new { f = 0, s = 1, nid = 202 },
            choiceEventData = new { sl = new long[] { 7 }, cs = 0, ri = 0 },
            dungeonUnitList = Array.Empty<object>(),
            updatedEgoGifts = Array.Empty<object>(),
        }));
        Assert.Equal(HttpStatusCode.OK, resp.StatusCode);
        ...
        {
            var result = doc.RootElement.GetProperty("result");
            var egs = result.GetProperty("currentEgoGifts").EnumerateArray().ToList();
            Assert.Contains(egs, e => e.GetProperty("id").GetInt64() == 993003);
            var dul = result.GetProperty("dungeonUnitList").EnumerateArray().ToList();
            Assert.All(dul, u => Assert.Equal(5, u.GetProperty("mlos").GetInt64())); // sbmlos 3 + egmlos 2
        }
    }
